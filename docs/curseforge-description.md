...
- **No inventing chores:** after finishing what you asked, the companion takes at most two more actions on its own initiative before waiting to be spoken to. Every finished command otherwise prompts it for a next step, which one instruction can chain indefinitely. `behavior.maxAutonomousTurns` tunes it; `0` removes the cap.
